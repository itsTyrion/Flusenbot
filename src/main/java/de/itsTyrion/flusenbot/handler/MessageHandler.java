    ...
    private void onMemberJoin(Message msg, User user, boolean captcha) {
        ...
        if (photos.totalCount() == 0) { // no profile photos or last name, might be a bot
            if (user.username() == null || user.username().isEmpty()) {
                Log.warning("Highly suspicious account joined (No last name/pic/username)");
            } else
                Log.warning("Suspicous account joined (No last name, no pic)");
        }

        if (captcha) {
            // mute new members until they pass the anti bot
            bot.execute(new RestrictChatMember(chatID, user.id()).untilDate(Utils.getUnixTime() + 21));
            val button = rand.nextInt(10); // get a random button, there's 9 and Telegram counts from 1 :rolleyes:
            ...
            val request = new SendMessage(chatID,
                    ...
                    "Bitte wähle das " + idText + " Emoji zum verifizieren. \n" +
                            "Du hast 20 Sek. Zeit. 2x Fehleingabe/Zeitüberschreitung = Tschüss (Anti Bot)")
            ...
            Utils.runDelayed(() -> { // If there'S not a been a correct input after 20 seconds, bye.
                if (UpdateHandler.map.containsKey(user.id()) && !cooldown.contains(chatID)) {
                    // I've never seen a bot rejoin. Also prevents users being flase banned for i.e. Telegram crashing
